...
The project ships a small CLI, a local web viewer, and an optional Codex skill.
The CLI collects and stores review artifacts in the OS cache; the viewer reads
those artifacts from a localhost-only server.

## Highlights

- Generate a review tour from the current branch, staged changes, or working tree.
- Open an interactive local viewer with chapters, file navigation, and diff review.
- Keep runtime artifacts out of the reviewed repository.
- Use deterministic file-based chapters by default, with an optional Codex skill
  flow for richer AI-authored chapters.
...
Then run it from the git repository you want to review:
...
By default, Review Tour infers the most useful diff mode, writes the
artifact to the OS cache, starts the local viewer if needed, and prints a URL.
...
Prints versioned skill content bundled with the installed CLI. This keeps the
distributed Vercel skills stub aligned with the CLI version:
...
Review Tour includes a Codex skill definition at
[`skills/review-tour/SKILL.md`](skills/review-tour/SKILL.md).
It follows the same CLI + skill pattern as
[`vercel-labs/agent-browser`](https://github.com/vercel-labs/agent-browser):
the distributed skill is a small stub, and the installed CLI serves the
versioned workflow with `review-tour skills get core`.

Install the skill globally for Codex:

```bash
npx skills add hiroingk/review-tour --skill review-tour --agent codex --global
```

The skills CLI chooses the supported Codex skill directory for the selected
scope and CLI version.

Install it into the current project instead:

```bash
npx skills add hiroingk/review-tour --skill review-tour --agent codex
```

List the skills exposed by this repository:

```bash
npx skills add hiroingk/review-tour --list
```

Use the skill prompt without installing it:

```bash
npx skills use hiroingk/review-tour --skill review-tour
```

The skill still requires the `review-tour` CLI to be installed on `PATH`:

```bash
pnpm run install:local
```

When the skill is installed, ask Codex from the repository you want to review:

```text
$review-tour Open the current diff as a guided review tour
```

Useful prompts:

```text
$review-tour Review staged changes in the right-pane browser
$review-tour Build a review tour with base origin/develop
$review-tour Open the current branch diff with AI-authored chapters
$review-tour Explain this diff in chat without opening the viewer
```

By default, the skill reviews the collected diff, creates AI-authored chapters,
and writes the official viewer artifact:
...
The deterministic one-command path is used by the CLI directly, or as a skill
fallback if AI chapter generation cannot complete:
