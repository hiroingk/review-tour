...
export function CommandDialogBackdrop({
  ...
  return (
    <CommandDialogPrimitive.Backdrop
      className={cn(
        'fixed inset-0 z-50 bg-black/32 backdrop-blur-sm transition-[opacity,background-color,backdrop-filter] duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0',
        className,
      )}
    ...
    <CommandDialogPortal {...portalProps}>
      ...
      <CommandDialogViewport>
        <CommandDialogPrimitive.Popup
          className={cn(
            'relative row-start-2 flex max-h-105 min-h-0 w-full min-w-0 max-w-xl -translate-y-[calc(1.25rem*var(--nested-dialogs))] scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-2xl border border-line bg-popover not-dark:bg-clip-padding text-popover-foreground opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-[0_24px_70px_rgb(0_0_0_/_42%)] outline-none transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:bg-transparent before:shadow-[inset_0_1px_0_rgb(255_255_255_/_3%)] data-nested:data-ending-style:translate-y-8 data-nested:data-starting-style:translate-y-8 data-nested-dialog-open:origin-top data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 **:data-[slot=scroll-area-viewport]:data-has-overflow-y:pe-1 dark:before:shadow-[inset_0_1px_0_rgb(255_255_255_/_3%)]',
            className,
          )}
  ...
  return (
    <div className="px-2.5 py-1.5">
      <AutocompleteInput
        ...
        size="lg"
        startAddon={<HugeiconsIcon color="currentColor" icon={Search01Icon} strokeWidth={1.7} />}
        {...props}
      />
    </div>
  );
...
export function CommandPanel({
  ...
  return (
    <div
      className={cn(
        'relative -mx-px not-has-[+[data-slot=command-footer]]:-mb-px min-h-0 rounded-t-xl not-has-[+[data-slot=command-footer]]:rounded-b-2xl border border-line border-b-0 bg-popover bg-clip-padding shadow-none [clip-path:inset(0_1px)] not-has-[+[data-slot=command-footer]]:[clip-path:inset(0_1px_1px_1px_round_0_0_calc(var(--radius-2xl)-1px)_calc(var(--radius-2xl)-1px))] before:pointer-events-none before:absolute before:inset-0 before:rounded-t-[calc(var(--radius-xl)-1px)] **:data-[slot=scroll-area-scrollbar]:mt-2',
        className,
      )}
      {...props}
    />
...
export function CommandSeparator({
  ...
  return (
    <AutocompleteSeparator
      className={cn('my-2 bg-line', className)}
...
export function CommandFooter({
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-between gap-2 rounded-b-[calc(var(--radius-2xl)-1px)] border-line border-t px-5 py-3 text-muted-foreground text-xs',
        className,
      )}
